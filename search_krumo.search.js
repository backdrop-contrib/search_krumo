    ...
    attach: function() {
      ...
      $('form#search-krumo').submit(function(e) {
        ...
        if (q) {
          ...
          $('.search-krumo-results').html(Drupal.formatPlural($('.krumo-element > a:contains('+ q +'), .krumo-element > strong:contains('+ q +')', k).length, 'Found 1 element', 'Found @count elements'));
        }
        else {
